...
# 웹 검색 도구 
class VerifyRagState(CorrectiveRagState):
...
def retrieve_documents(state: VerifyRagState) -> VerifyRagState:
    print("---문서 검색---")
    query = state.get("rewritten_query", state["question"])
    docs = web_search.invoke(query)
    return {"documents": docs}

def extract_and_evaluate_information(state: VerifyRagState) -> VerifyRagState:
    print("---정보 추출 및 평가---")
    extracted_strips = []

    for doc in state["documents"]:
        extract_prompt = ChatPromptTemplate.from_messages([
            ("system", """당신은 제품 정보에 대해 진위 여부를 판단하는 전문가입니다. 주어진 문서에서 질문과 관련된 주요 사실과 정보를 3~5개 정도 추출하세요. 
...
def rewrite_query(state: VerifyRagState) -> VerifyRagState:
    print("---쿼리 재작성---")
...
def generate_node_answer(state: VerifyRagState) -> VerifyRagState:
    print("---답변 생성---")
...
def should_continue(state: VerifyRagState) -> Literal["계속", "종료"]:
...
def make_verify_workflow():
    

    # 그래프 생성
    workflow = StateGraph(VerifyRagState)
    ...
    # 그래프 컴파일
    verify_web_agent = workflow.compile()

    return verify_web_agent