...
# 웹 검색 도구 
class ReviewRagState(CorrectiveRagState):
    rewritten_query: str   # 재작성한 질문 
    extracted_info: Optional[ExtractedInformation]   # 추출된 정보 조각 
    node_answer: Optional[str] 

def retrieve_documents(state: ReviewRagState) -> ReviewRagState:
    print("---문서 검색---")
    query = state.get("rewritten_query", state["question"])
    docs = web_search.invoke(query)
    return {"documents": docs}

def extract_and_evaluate_information(state: ReviewRagState) -> ReviewRagState:
    print("---정보 추출 및 평가---")
    extracted_strips = []

    for doc in state["documents"]:
        extract_prompt = ChatPromptTemplate.from_messages([
            ("system", """당신은 게임기의 상품 리뷰, 사용자 경험, 리콜 이력을 판단하는 전문가입니다. 주어진 문서에서 질문과 관련된 주요 사실과 정보를 3~5개 정도 추출하세요. 
...
def rewrite_query(state: ReviewRagState) -> ReviewRagState:
    print("---쿼리 재작성---")
...
def generate_node_answer(state: ReviewRagState) -> ReviewRagState:
    print("---답변 생성---")

    answer_prompt = ChatPromptTemplate.from_messages([
        ("system", """당신은 게임기 리뷰, 사용자 경험, 리콜 이력 검색 전문가입니다. 주어진 질문과 추출된 정보를 바탕으로 답변을 생성해주세요. 
...
def should_continue(state: ReviewRagState) -> Literal["계속", "종료"]:
...
def make_review_workflow():
    # 그래프 생성
    workflow = StateGraph(ReviewRagState)
    ...
    # 그래프 컴파일
    review_web_agent = workflow.compile()
    return review_web_agent